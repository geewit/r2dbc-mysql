...
final class HandshakeResponse41 extends ScalarClientMessage implements HandshakeResponse {
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof HandshakeResponse41 that) {
            return zstdCompressionLevel == that.zstdCompressionLevel
    ...
    public String toString() {
        return "HandshakeResponse41{capability=" + header.getCapability()
                + ", collationId=" + header.getCollationId()
                + ", user='" + user
    ...
    private void writeAttrs(ByteBuf buf, Charset charset) {
        ...
        try {
            attributes.forEach((key, value) -> {
                writeVarIntString(attributesBuf, key, charset);
                writeVarIntString(attributesBuf, value, charset);
            });
