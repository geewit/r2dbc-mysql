...
final class BitSetCodec extends AbstractClassedCodec<BitSet> {
    ...
    private static final class BitSetMySqlParameter extends AbstractMySqlParameter {
        ...
        public Mono<ByteBuf> publishBinary(final ByteBufAllocator allocator) {
            return switch (type) {
                case TINYINT -> Mono.fromSupplier(() -> allocator.buffer(Byte.BYTES).writeByte((int) value));
                case SMALLINT -> Mono.fromSupplier(() -> allocator.buffer(Short.BYTES).writeShortLE((int) value));
                case INT -> Mono.fromSupplier(() -> allocator.buffer(Integer.BYTES).writeIntLE((int) value));
                default -> // BIGINT
                        Mono.fromSupplier(() -> allocator.buffer(Long.BYTES).writeLongLE(value));
            };
        }
        ...
        public boolean equals(Object o) {
            ...
            if (o instanceof BitSetMySqlParameter that) {
