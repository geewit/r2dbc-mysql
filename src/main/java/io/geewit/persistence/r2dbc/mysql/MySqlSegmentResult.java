...
final class MySqlSegmentResult implements MySqlResult {
    ...
    public Mono<Long> getRowsUpdated() {
        return segments.<Long>handle((segment, sink) -> {
            if (segment instanceof UpdateCount updateCount) {
                sink.next(updateCount.value());
            } else if (segment instanceof Message message) {
    ...
    public <T> Flux<T> map(Function<? super Readable, ? extends T> f) {
        ...
        return segments.handle((segment, sink) -> {
            if (segment instanceof RowSegment rowSegment) {
                try {
                    sink.next(f.apply(rowSegment.row()));
                } finally {
                    ReferenceCountUtil.safeRelease(rowSegment);
                }
            } else if (segment instanceof Message message) {
                sink.error(message.exception());
            } else if (segment instanceof ReferenceCounted) {
    ...
    static MySqlResult toResult(boolean binary,
                                Client client,
                                Codecs codecs,
                                String syntheticKeyName,
    ...
    private static final class MySqlSegments implements BiConsumer<ServerMessage, SynchronousSink<Segment>> {
        ...
        private MySqlSegments(boolean binary, Client client, Codecs codecs, String syntheticKeyName) {
            this.binary = binary;
            this.client = client;
            this.codecs = codecs;
        ...
        public void accept(ServerMessage message, SynchronousSink<Segment> sink) {
            switch (message) {
                ...
                case SyntheticMetadataMessage syntheticMetadataMessage -> {
                    DefinitionMetadataMessage[] metadataMessages = syntheticMetadataMessage.unwrap();
                ...
                case ErrorMessage errorMessage -> sink.next(new MySqlMessage(errorMessage));
