...
import io.geewit.persistence.r2dbc.mysql.message.client.SslRequest;
import io.geewit.persistence.r2dbc.mysql.message.server.*;
...
final class MessageDuplexCodec extends ByteToMessageDecoder implements ChannelOutboundHandler {
    ...
    public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) {
        if (msg instanceof ClientMessage clientMessage) {
            ...
            Flux<ByteBuf> encoded = Flux.from(clientMessage.encode(allocator, this.context));

            OperatorUtils.envelope(encoded, allocator, sequenceId, clientMessage.isCumulative())
                    .subscribe(new WriteSubscriber(ctx, promise));

            switch (clientMessage) {
                case PrepareQueryMessage _ -> setDecodeContext(DecodeContext.prepareQuery());
                case PreparedFetchMessage _ -> setDecodeContext(DecodeContext.fetch());
                case SslRequest _ -> ctx.channel().pipeline().fireUserEventTriggered(SslState.BRIDGING);
                default -> {
                }
            }
        } else {
    ...
    public void userEventTriggered(ChannelHandlerContext ctx, Object evt) {
        if (evt instanceof PacketEvent packetEvent) {
            switch (packetEvent) {
                case RESET_SEQUENCE:
    ...
    private void handleDecoded(List<Object> out, ServerMessage msg) {
        if (msg instanceof ServerStatusMessage serverStatusMessage) {
            this.context.setServerStatuses(serverStatusMessage.getServerStatuses());
        }

        if (msg instanceof CompleteMessage) {
            // Metadata EOF message will be not receive in here.
            setDecodeContext(DecodeContext.command());
        } else if (msg instanceof SyntheticMetadataMessage syntheticMetadataMessage) {
            if (syntheticMetadataMessage.isCompleted()) {
                setDecodeContext(DecodeContext.command());
            }
        } else if (msg instanceof ColumnCountMessage columnCountMessage) {
            setDecodeContext(DecodeContext.result(this.context.getCapability().isEofDeprecated(),
                    columnCountMessage.getTotalColumns()));
            return; // Done, no need use generic handle.
        } else if (msg instanceof PreparedOkMessage message) {
