...
import io.geewit.persistence.r2dbc.mysql.api.MySqlBatch;
import io.geewit.persistence.r2dbc.mysql.api.MySqlTransactionDefinition;
import io.geewit.persistence.r2dbc.mysql.client.Client;
import io.geewit.persistence.r2dbc.mysql.client.FluxExchangeable;
import io.geewit.persistence.r2dbc.mysql.constant.ServerStatuses;
...
import io.geewit.persistence.r2dbc.mysql.message.client.PingMessage;
import io.geewit.persistence.r2dbc.mysql.message.client.PrepareQueryMessage;
import io.geewit.persistence.r2dbc.mysql.message.client.PreparedCloseMessage;
import io.geewit.persistence.r2dbc.mysql.message.client.PreparedExecuteMessage;
import io.geewit.persistence.r2dbc.mysql.message.client.PreparedFetchMessage;
import io.geewit.persistence.r2dbc.mysql.message.client.PreparedResetMessage;
import io.geewit.persistence.r2dbc.mysql.message.client.PreparedTextQueryMessage;
import io.geewit.persistence.r2dbc.mysql.message.client.TextQueryMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.CompleteMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.EofMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.ErrorMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.LocalInfileRequest;
import io.geewit.persistence.r2dbc.mysql.message.server.OkMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.PreparedOkMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.ServerMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.ServerStatusMessage;
import io.geewit.persistence.r2dbc.mysql.message.server.SyntheticMetadataMessage;
...
final class QueryFlow {
    ...
    private static final BiConsumer<ServerMessage, SynchronousSink<ServerMessage>> PING = (message, sink) -> {
        if (message instanceof ErrorMessage) {
            sink.next(message);
            sink.complete();
        } else if (message instanceof CompleteMessage completeMessage && completeMessage.isDone()) {
            sink.next(message);
            sink.complete();
        } else {
            ReferenceCountUtil.safeRelease(message);
    ...
    static Flux<Flux<ServerMessage>> execute(Client client, List<String> statements) {
        return Flux.defer(() -> switch (statements.size()) {
            case 0 -> Flux.empty();
            case 1 -> execute0(client, statements.getFirst()).windowUntil(RESULT_DONE);
            default -> client.exchange(new MultiQueryExchangeable(statements.iterator()))
                    .windowUntil(RESULT_DONE);
        });
    }
    ...
    static Mono<Void> executeVoid(Client client, String sql) {
        return Mono.defer(() -> client.<ServerMessage>exchange(new TextQueryMessage(sql), (message, sink) -> {
            if (message instanceof ErrorMessage errorMessage) {
                sink.next(errorMessage.offendedBy(sql));
                sink.complete();
            } else {
                sink.next(message);

                if (message instanceof CompleteMessage completeMessage && completeMessage.isDone()) {
                    sink.complete();
                }
            }
    ...
    static Mono<Void> setSessionVariables(Client client, List<String> sessionVariables) {
        switch (sessionVariables.size()) {
            case 0:
                return Mono.empty();
            case 1:
                return setSessionVariable(client, sessionVariables.getFirst());
            default: {
...
abstract class BaseFluxExchangeable extends FluxExchangeable<ServerMessage> {
    ...
    public final void accept(ServerMessage message, SynchronousSink<ServerMessage> sink) {
        if (message instanceof ErrorMessage errorMessage) {
            sink.next(errorMessage.offendedBy(offendingSql()));
            sink.complete();
        } else if (message instanceof LocalInfileRequest request) {
    ...
    protected abstract void tryNextOrComplete(SynchronousSink<ServerMessage> sink);
...
final class SimpleQueryExchangeable extends BaseFluxExchangeable {
    ...
    @Override
    protected void tryNextOrComplete(SynchronousSink<ServerMessage> sink) {
...
final class PrepareExchangeable extends FluxExchangeable<ServerMessage> {
        ...
        switch (mode) {
            case PREPARE_OR_RESET:
                switch (message) {
                    ...
                    case SyntheticMetadataMessage syntheticMetadataMessage when syntheticMetadataMessage.isCompleted() -> {
                        Integer statementId = this.statementId;
                        if (statementId == null) {
                    ...
                    case null, default -> ReferenceCountUtil.safeRelease(message);
                }
            ...
            case EXECUTE:
                if (message instanceof CompleteMessage completeMessage && completeMessage.isDone()) {
                    ...
                    onCompleteMessage(completeMessage, sink);
                } else if (message instanceof SyntheticMetadataMessage syntheticMetadataMessage) {
                    EofMessage eof = syntheticMetadataMessage.getEof();
                    if (eof instanceof ServerStatusMessage serverStatusMessage) {
                        // Otherwise, cursor does not be opened, wait for end of row EOF message.
                        if ((serverStatusMessage.getServerStatuses() & ServerStatuses.CURSOR_EXISTS) != 0) {
                            if (doNextFetch(sink)) {
                                sink.next(message);
                            }
    ...
    private void onCompleteMessage(CompleteMessage message, SynchronousSink<ServerMessage> sink) {
        ...
        if (message instanceof ServerStatusMessage serverStatusMessage) {
            short statuses = serverStatusMessage.getServerStatuses();
