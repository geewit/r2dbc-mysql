    ...
    public static long crossReadVarInt(ByteBuf firstPart, ByteBuf secondPart) {
        ...
        if (firstByte < VAR_INT_2_BYTE_CODE) {
            ...
            return switch (readable) {
                case 0 -> secondPart.readUnsignedShortLE();
                case 1 -> firstPart.readUnsignedByte() | (secondPart.readUnsignedByte() << Byte.SIZE);
                default -> firstPart.readUnsignedShortLE();
            };
        } else if (firstByte == VAR_INT_3_BYTE_CODE) {
            int readable = firstPart.readableBytes();

            return switch (readable) {
                case 0 -> secondPart.readUnsignedMediumLE();
                case 1 -> firstPart.readUnsignedByte() |
                        (((long) secondPart.readUnsignedShortLE()) << Byte.SIZE);
                case 2 -> firstPart.readUnsignedShortLE() |
                        (((long) secondPart.readUnsignedByte()) << Short.SIZE);
                default -> firstPart.readUnsignedMediumLE();
            };
        }
