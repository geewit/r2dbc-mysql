...
import io.geewit.persistence.r2dbc.mysql.cache.PrepareCache;
...
import io.geewit.persistence.r2dbc.mysql.codec.CodecsBuilder;
...
import io.geewit.persistence.r2dbc.mysql.message.client.*;
...
final class InitFlow {
    ...
    private static final BiConsumer<ServerMessage, SynchronousSink<Boolean>> INIT_DB = (message, sink) -> {
        if (message instanceof ErrorMessage msg) {
    ...
    private static final BiConsumer<ServerMessage, SynchronousSink<Void>> INIT_DB_AFTER = (message, sink) -> {
        if (message instanceof ErrorMessage errorMessage) {
            sink.error(errorMessage.toException());
        } else if (message instanceof CompleteMessage completeMessage && completeMessage.isDone()) {
            sink.complete();
        } else {
            ReferenceCountUtil.safeRelease(message);
     ...
     */
    static Mono<Void> initHandshake(Client client,
                                    SslMode sslMode,
                                    String database,
                                    String user,
    ...
    private static IsolationLevel convertIsolationLevel(String name) {
        if (name == null) {
    ...
    private static final class SessionState {
        ...
        SessionState(IsolationLevel level,
                     String product,
                     ZoneId timeZone) {
        ...
        public boolean equals(Object o) {
            ...
            if (o instanceof SessionState that) {
...
final class HandshakeExchangeable extends FluxExchangeable<Void> {
    ...
    HandshakeExchangeable(Client client,
                          SslMode sslMode,
                          String database,
                          String user,
                          CharSequence password,
                          Set<CompressionAlgorithm> compressions,
                          int zstdCompressionLevel) {
        this.client = client;
    ...
    public void accept(ServerMessage message, SynchronousSink<Void> sink) {
        ...
        switch (message) {
            ...
            case ChangeAuthMessage msg -> {

                authProvider = MySqlAuthProvider.build(msg.getAuthType());
                salt = msg.getSalt();
                emitNext(createAuthResponse("change authentication"), sink);
            }
            case null, default -> sink.error(new R2dbcPermissionDeniedException("Unexpected message type '" +
                    Objects.requireNonNull(message).getClass().getSimpleName() + "' in login phase"));
        }
    }
