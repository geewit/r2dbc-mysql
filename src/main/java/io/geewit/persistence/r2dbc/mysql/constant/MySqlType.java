...
import io.geewit.persistence.r2dbc.mysql.api.MySqlNativeTypeMetadata;
...
public enum MySqlType implements Type {
    ...
    public static MySqlType of(MySqlNativeTypeMetadata metadata) {
        ...
        return switch (metadata.getTypeId()) {
            case ID_DECIMAL, ID_NEW_DECIMAL -> DECIMAL;
            case ID_TINYINT -> metadata.isUnsigned() ? TINYINT_UNSIGNED : TINYINT;
            case ID_SMALLINT -> metadata.isUnsigned() ? SMALLINT_UNSIGNED : SMALLINT;
            case ID_INT -> metadata.isUnsigned() ? INT_UNSIGNED : INT;
            case ID_FLOAT -> FLOAT;
            case ID_DOUBLE -> DOUBLE;
            case ID_NULL -> NULL;
            case ID_TIMESTAMP -> TIMESTAMP;
            case ID_BIGINT -> metadata.isUnsigned() ? BIGINT_UNSIGNED : BIGINT;
            case ID_MEDIUMINT -> metadata.isUnsigned() ? MEDIUMINT_UNSIGNED : MEDIUMINT;
            case ID_DATE -> DATE;
            case ID_TIME -> TIME;
            case ID_DATETIME -> DATETIME;
            case ID_YEAR -> YEAR;
            case ID_VARCHAR, ID_VAR_STRING, ID_STRING -> metadata.isBinary() ? VARBINARY : VARCHAR;
            case ID_BIT -> BIT;
            case ID_JSON -> JSON;
            case ID_ENUM -> ENUM;
            case ID_SET -> SET;
            case ID_TINYBLOB -> metadata.isBinary() ? TINYBLOB : TINYTEXT;
            case ID_MEDIUMBLOB -> metadata.isBinary() ? MEDIUMBLOB : MEDIUMTEXT;
            case ID_LONGBLOB -> metadata.isBinary() ? LONGBLOB : LONGTEXT;
            case ID_BLOB -> metadata.isBinary() ? BLOB : TEXT;
            case ID_GEOMETRY ->
